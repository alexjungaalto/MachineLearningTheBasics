...
the training set. If the training error has can serve as such an indicator for 
general performance depends on the statistical properties of 
the datapoints and on the hypothesis space used by the ML method.  
...
high-dimensional hypotheses spaces constituted by maps represented 
by deep neural networks with billions of tunable weights. For such vast 
hypothesis spaces, it is quite easy to find some hypothesis that fits well the 
training data and has small training error. However, such a hypothesis might overfit 
the training data and does a poor job on datapoints outside the training data. 
...
$\vx$. Section \ref{sec_overfitting_sec_6} analyzes overfitting of 
linear regression by using a probabilistic model for datapoints. 

This chapter discusses how a simple procedure known as 
validation can detect and avoid overfitting. Section \ref{sec_validate_predictor} 
will discuss how {\bf to validate} a learnt hypothesis by computing its average loss on 
datapoints which are different from the training set. The datapoints 
used to validate the hypothesis are referred to as the {\bf validation set}. 
If a ML method overfits, it will learn a hypothesis whose training error is 
much smaller than the validation error. 
...
or risk of a hypothesis. This analysis sheds some light on the interplay 
between the properties of the hypothesis space used in a ML method 
with the resulting training and validation error.
...
will show how we can debug and diagnose ML methods merely by 
comparing its training and validation error. The comparison of these 
two errors informs possible improvements of ML methods by 
collecting more data or change the model (hypothesis space). 
...
A very simple recipe for validating a ML method is as follows (see Figure \ref{fig_split_train_val}): 
\begin{enumerate} 
\item randomly divide (``split'') the entire set $\dataset$ of labeled datapoints into 
...
\item learn a hypothesis $\hat{h}$ via ERM using the training data $\trainset$, i.e., 
compute (cf.\ \eqref{equ_def_ERM_funs}) 
\begin{align} 
...
\end{equation}  
\item validate the hypothesis $\hat{h}$ obtained from \eqref{equ_def_hat_h_fitting} by computing the empirical risk 
\begin{equation} 
...
with some benchmark error level $E_{0}$, we can get 
some idea of how to adapt components 
of the ML method to improve performance: 
\begin{itemize} 
...
reason for $\emperror(\hat{h}| \trainset) \gg \emperror(\hat{h}| \valset)$ 
...
%ncurred by a predictor when applied to 
%some labeled datapoints (training data) $\dataset= \{ \vz^{(\sampleidx)} = (\vx^{(\sampleidx)},y^{(\sampleidx)}) %\}_{\sampleidx=1}^{\samplesize}$, 
is a good indicator for the general or average prediction performance of a hypothesis. % prediction error (see \eqref{equ_decomp_E_pred_toy_model}) 
