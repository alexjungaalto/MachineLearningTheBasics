...
\caption{Illustration of $k$-fold CV for $k=5$. We evenly partition the entire dataset $\dataset$ into $k=5$ subsets 
	(or folds) of the same size. We then repeat the validated ERM Algorithm \ref{alg:validated_ERM} for $k=5$ times. 
    The $\foldidx$th repetition uses the $\foldidx$th fold as validation set and the remaining $k\!-\!1(=4)$ folds as training set.}
\label{fig_k_fold_CV}
\end{figure}
...
\begin{algorithm}[htbp]
	...
	%	\State \texttt{<do stuff>}
		...
		\begin{equation} 
		\label{equ_def_training_error_val}
		\trainerror^{(\foldidx)} \defeq \emperror\big(\hat{h}| \trainset \big) = (1/\samplesize_{t}) \sum_{\sampleidx=1}^{\samplesize_{t}} \loss{(\vx^{(\sampleidx)},y^{(\sampleidx)})}{h}. 
		\end{equation} 
		\State compute {\bf validation error}
		\begin{equation} 
		\label{equ_def_training_val_val}
		\valerror^{(\foldidx)} \defeq \emperror\big(\hat{h}| \valset \big)=   (1/\samplesize_{v}) \sum_{\sampleidx=\samplesize_{t}+1}^{\samplesize} \loss{(\vx^{(\sampleidx)},y^{(\sampleidx)})}{\hat{h}}. 
		\end{equation} 
		\EndFor
		...
		\Ensure learnt hypothesis $\hat{h}^{(\foldidx)}$ for each fold $\foldidx=1,\ldots,k$; average training error $E_{t}$; average validation error $E_{v}$
	\end{algorithmic}
\end{algorithm}
...
between different candidate models $\hypospace_{1},\hypospace_{2},\ldots,\hypospace_{M}$. 

%Chapter \ref{ch_Elements_ML} discussed that the choice of the hypothesis space or model is a design choice. 
I%t might be not obvious what a good choice for the hypothesis space is. A good strategy is often to try out several 
...
Different choices for the maximum degree $\featuredim$ yield a different hypothesis space: $\hypospace_{1} = \mathcal{H}_{\rm poly}^{(0)},\hypospace_{2} = \mathcal{H}_{\rm poly}^{(1)},\ldots,\hypospace_{M} = \hypospace_{\rm poly}^{(M-1)}$. 
...
result in different hypothesis spaces. For example, $\hypospace_{1} = \mathcal{H}^{(2)}_{\rm Gauss}$ with $\sigma=1$ and $\mu_{1}=1$ and $\mu_{2}=2$, 
$\hypospace_{2} = \hypospace^{(2)}_{\rm Gauss}$ with $\sigma = 1/10$, $\mu_{1}=10$, $\mu_{2}= 20$.

A principled approach for choosing between different candidate models
$\hypospace_{1},\hypospace_{2},\ldots,\hypospace_{M}$ is to learn and validate, for each 
hypothesis space, a hypothesis. Then, we choose the hypothesis from those model 
which resulted in the smallest validation error. 
\begin{itemize} 
...
and SVM. Both methods learn a linear hypothesis but using two different loss functions, logistic 
loss and the hinge loss. We could then compute the validation errors using those two loss functions. 
However, it might then be difficult to compare these two methods since their validation errors are 
measured in different loss functions. We could instead compute the validation error for both methods, 
logistic regression and SVM, using the $0/1$ loss (accuracy). The resulting average loss (accuracy) on the 
validation set can then be compared directly. 
