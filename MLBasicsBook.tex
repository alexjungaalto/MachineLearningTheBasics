...
for the feedback received from Jukka Suomela, V{\"a}ini{\"o} Mehtola, 
...
%data. Consider the data point $\vz$ representing some day with minimum 
...
data points but also to represent models for the data. One such class of 
...
a (predictor or classifier) map that reads in features of a data point 
...
Besides the size of datasets, another computational challenge 
for modern ML methods is that they must be able to fit highly 
non-linear predictor maps. Deep learning methods address this 
challenge by using a computationally convenient representation 
of non-linear maps via artificial neural networks \cite{Goodfellow-et-al-2016}). 
...
when its data points can be grouped into a few coherent subsets 
of cluster (see Chapter \ref{ch_Clustering}). Another example for 
such an intrinsic structure is when the data points are localized 
...
moving direction for a toy car given its current state. Data points represent 
a particular state of the car, its label is the optimum steering direction. 

It is typically impossible to get labeled data points here since there 
...
of highly relevant features to characterize data points. If we use too 
...
to predict the label of a data point. Chapter \ref{ch_FeatureLearning} 
discusses {\bf feature learning} methods that automatically determine 
the most relevant features out of the ``raw features'' of a data point. 

%These methods exploit 
%intrinsic correlations and redundancy in the raw data to construct 
%few relevant features for data points. These relevant features can 
...
customers.  Data points might represent time periods, animals, 
...
datapoints it contains. The number of data points within a dataset is 
...
\begin{figure}[htbp]
	...
	\caption{Two main parameters of a dataset are the number 
		$\samplesize$ of data points it contains and the number 
		$\featuredim$ of features used to characterize individual 
		data points. A very important characteristic of a dataset 
...
every single microscopic property of a data point. Consider a 
data point that represents a vaccine. A full characterization of 
such a data point would require to specify its chemical composition 
...
blurry. The same property of a data point might be used 
...
to be used as features is a design choice. As the features of a data point, 
we typically use properties or quantities that can be computed or 
measured easily. This is only an informal characterization since there 
is no widely-applicable measure for the difficulty in measuring a specific 
property. 
...
features that should be used to characterize the data points 
arising in a given application. As a rule of thumb, the number $\samplesize$ 
of (labeled) data points provided to a ML method should be 
...
by either collecting a sufficiently large number $\samplesize$ of data points 
...
The acquisition of (labeled) data points might be costly, requiring human 
...
As an example for an intrinsic symmetry of data, consider data points 
...
Instead of collecting more data points to ensure the condition $\samplesize/\featurelen$, 
we can use try to reduce the number $\featurelen$ of features used to 
characterize data points. In some applications, we might use some domain 
...
for extracting a small number of relevant features of data points. 
...
A data point is typically characterized by several individual features 
...
\end{equation}
Each data point is then characterized by such a feature vector $\vx$. 
The feature vectors of data points take on values out of some {\bf feature space}, 
which we denote as $\featurespace$. 

The feature space is a design choice as it depends on what properties of a 
data point we use as its features. This design choice should take into account 
...
%This can be useful for {\bf network-structured datasets} where the data points can be 
...
with dimension $\featuredim$ being the number of features of a data point. This 
...
Consider data points representing images with $512$ by $512$ pixels. 
A natural construction for the feature vector of such data points is to 
...
In some applications, it is less obvious how to represent data points 
...
start of which the snapshot in Figure \ref{fig:image} has
 ...
 The label $y$ associated with a data point (which represents 
...
possible label values of data points. We refer to ML problems involving 
...
on data points whose label $y$ takes on values $-1$ or $1$. We 
...
as marking images which show a cat. 
...
and label $y$. 
%Each data point represents a particular day whose 
...
the data points in a scatter plot.
...
It might seem somewhat awkward to interpret data points as realizations 
...
to characterize the statistical properties of collections of data points. 

The probability distribution $p(x)$ is either assumed known or estimated 
from data (see Section \ref{sec_max_iikelihood}). It is often enough to not 
...
%$\vx^{(\sampleidx)}$ of a data points to a predicted label $\hat{y} = h(\vx)$. 
...
a symbol denoting the set of all maps from $\labelspace$ to $\labelspace$. 
Despite its appearance, $\labelspace^{\featurespace}$ does not 
refer to a power like $2^5$. 
...
The decision regions \eqref{equ_decision_region} of a linear classifier are 
half-spaces and their decision boundary is a hyperplane $\{ \vx: \vw^{T} \vx = b \}$ 
(see Figure \ref{fig_lin_dec_boundary}).
...
and label $y$ of a data point is highly non-linear. We can then upgrade the 
...
%Consider data points being images of animals with features $\vx$ 
...
only be used for data points whose features are numeric 
...
%As with the choice of what properties to use as features of a data point, 
...
data points using the feature space $\featurespace = \{0,1\}^{1000}$ 
...
Consider $\samplesize$ data points, each characterized by a feature 
vector $\vx^{(\sampleidx)} \in \mathbb{R}^{\featuredim}$ and a numeric label 
$y^{(\sampleidx)} \in \mathbb{R}$. We assume that data points are 
...
know the feature $\vx$ and the label $y$ of the data point.}
...
consisting of $4$ data points with binary labels. Data points 
with label $y=1$ are depicted by squares, while those with label 
$y=-1$ are depicted by circles. We could use these four data points 
to learn a linear hypothesis $h(x) = w_{1} x + w_{0}$ to classify 
data points according to $\hat{y} = 1$ for $h(x) \geq 0$ and 
$\hat{y}=-1$ for $h(x) < 0$. 
...
perfectly match the labels of the four training data points. In 
contrast, the classifications $\hat{y}$ obtained by thresholding 
$h^{(1)}(x)$ disagree with the true labels for the data points 
with $y=-1$. 

Based on the training data, we should prefer using $h^{(2)}(x)$ 
over $h^{(1)}$ to classify data points. However, the squared error 
...
the quality of a hypothesis map that is used for classifying data points 
...
used to classify data points according to their binary labels. 
...
Ideally we would like to have $\hat{y} = y$ for any data point. 
...
for a data point with features $\vx$ and label $y=1$ as a function of 
the hypothesis $h$. For a given data point $(\vx,y)$ and hypothesis 
...
data points are interpreted as realizations of i.i.d.\ random variables 
...
use more sophisticated optimization methods to learn 
...
\begin{center}
  ...
  ymax=2.5
     ...
     \vspace*{-10mm}
\end{center}
\caption{Three loss functions for assessing the quality of a hypothesis $h$ which 
	is used to classify a data point with label $y=1$ according to \eqref{equ_def_classification}. 
	The more confident we are in a correct decision, i.e, the more positive $h(x)$, 
	the smaller the loss. Each of the three loss functions tends monotonically to $0$ 
	for increasing $h(x)$.}
...
\framebox[0.96\textwidth]{
 \parbox{0.9\textwidth}{
An important aspect guiding the choice for the loss function is the 
...
(average) loss. The difficulty of the resulting optimization 
problem (see Chapter \ref{ch_Optimization}) depends crucially 
on the properties of the chosen loss function. Some loss functions 
can be minimized using efficient iterative methods (see Chapter \ref{ch_GD}). 
}}
\end{center}
...
Some applications involve data points whose labels are so 
...
\label{ex_2_8}
Consider data points whose features are $10 \times 10$ 
black-and-white (bw) pixel images. Each data point is also 
...
\end{equation}
The family \eqref{equ_basis_Gaussian} of maps is parametrized by 
...
by a weight vector $\vw \in \mathbb{R}^{\featuredim}$. Each element 
of $\hypospace_{\rm Gauss}$ corresponds to a particular choice 
for the weight vector $\vw$. Instead of searching over $\hypospace_{\rm Gauss}$ 
...
Logistic regression is a method for classifying data points which 
...
We will shortly that even for binary labels it is quite useful to use a hypothesis 
map $h$ which can take on arbitrary real numbers. 

Note that we can always threshold the prediction $h(\vx)$ to obtain a 
predicted label $\hat{y} \in \{-1,1\}$. A data point with features $\vx$, 
...
Consider two data points with features $\vx^{(1)}, \vx^{(2)}$ and 
a linear classifier map $h$ yielding the function values $h(\vx^{(1)}) = 1/10$ 
and $h(\vx^{(2)}) = 100$. Whereas the predictions for both data points result 
in the same label predictions $\hat{y}^{(1)}\!=\!\hat{y}^{(2)}\!=\!1$, the classification 
of the data point with feature vector $\vx^{(2)}$ seems to be much more reliable. 
...
minimizes \eqref{equ_def_emp_risk_logreg}, we classify a data point 
...
(see Figure \ref{fig_lin_dec_boundary}). Any data point with features 
$\vx \in \mathcal{R}_{1}$ ($\vx \in \mathcal{R}_{-1}$) is classified as 
$\hat{y}\!=\!1$ ($\hat{y}\!=\!-1$). 

Logistic regression can be interpreted as a maximum likelihood 
estimator within a particular probabilistic model for the data points. 
This interpretation is based on modelling the label $y \in \{-1,1\}$ of a 
data point as random variables with the probability %\prob{y\!=\!1}$  
\begin{align}
...
is parametrized by the weight vector $\vw$ of the linear hypothesis $h^{(\vw)}(\vx)\!=\!\vw^{T} \vx$. 
...
from observed data points. A principled approach to estimate the weight 
...
as realizations of i.i.d.\ data points whose labels are distributed 
...
assess the usefulness of a hypothesis map $h \in \hypospace$ for classifying data points. 
...
Note that this is the same hypothesis space as used by linear and logistic regression 
...
is typically determined by few data points (such as $\vx^{(6)}$ in 
Figure \ref{fig_svm}) which are closest to the decision boundary. 
These data points have minimum distance to the decision boundary 
...
%the remaining data points become irrelevant for learning the classifier $h^{(\vw_{\rm SVM})}$. 
...
trees which are not too deep. Consider the hypothesis 
space consisting of all decision trees which uses the tests ``$\| \vx - \vu \| \leq r$'' and  
``$\| \vx - \vv \| \leq r$'' , with some vectors $\vu$ and $\vv$ and radius $r >0$. 
...
regressions (for discrete labels). 
...
data points are whose datasets; labels are correct partitioning/clustering; loss function is some notion of purity of 
clustering error; discuss partitional vs. hierarchical clustering (different choice for hypospace?)
...
The notion of similarity between data points can be based on physical 
...
in the form of labels associated with individual data points. In a 
social network, we might define the personal preference for some 
product as the label associated with a data point (which represents 
a user profile). 

Acquiring labels is often costly and requires manual labor or 
experiment design. Therefore, we assume to have access to 
the labels of only a few data points which belong to a small ``training set''. 
...
of close-by data points are similar, which allows to combine partially 
...
Most methods for processing graph signals rely on a signal model 
which are inspired by a cluster assumption \cite{SemiSupervisedBook}. 
...
%which serves as a proxy for the average prediction error. The resulting ML methods are optimization 
 ...
 over a given set of labeled data points,  
\begin{align}
...
when applied to new data points which are not contained in $\dataset$. 
...
Given a parameterization $h^{(\vw)}(\vx)$ of the predictor maps in the 
hypothesis space $\hypospace$, such as within linear regression 
\eqref{equ_def_hypo_linear_pred} or for ANNs (see Figure \ref{fig_ANN}), % within a liner regression model, i.e., $h(\vx^{(\timeidx)}) = \vw^{T}\vx^{(\timeidx)}$, 
...
by $h^{(\vw)}$ when applied to the data points in the dataset $\dataset$. 
Note that the two formulations \eqref{eq_def_ERM_weight} and \eqref{equ_def_ERM_funs} 
...
%This principle of learning by supervision, using labeled data points (``training examples''), 
...
Consider data points characterized by single numeric feature $x$ 
...
We call the set of data points used for validation, the {\bf validation set}. 
...
the training data points are linearly independent. It can be shown 
...
for the training (see \eqref{equ_def_ERM_funs}) and the validation 
of a predictor $\hat{h}$!
\end{quote}
}}
\end{center}
...
Consider data points with a non-linear relation between its 
...
Within our toy model, which relates the features $\vx$ of a data point 
...
can be influenced by varying the model complexity $r$ (see Figure \ref{fig_bias_variance}) 
which is a design parameter. The noise variance $\sigma^{2}$ is the 
intrinsic accuracy limit of our toy model \eqref{equ_linear_obs_model} 
...
error \eqref{equ_decomp_E_pred_toy_model} achieved by linear regression only 
applies if the observed data points are well modelled as realizations of random 
vectors according to \eqref{equ_linear_obs_model}, \eqref{equ_toy_model_iid} 
and \eqref{equ_labels_training_data}. The usefulness of this model for the 
data arising in a particular application has to be verified in practice by 
...
data points, their features and labels, a model (hypothesis space)
...
As an example, consider data points $\vz^{(\sampleidx)}$ obtained 
from historic recordings of Bitcoin statistics. Each data point $\vz^{(\sampleidx)} \in \mathbb{R}^{6}$ 
...
random projections blur features of individual data points 
...
&\sampleidx  &\quad &\mbox{Generic index used to enumerate data points in a list of data points. } \nonumber \\
...
&y  &\quad &\mbox{The label of some data point.} \nonumber \\ 
&\big(\vx^{(\sampleidx)},y^{(\sampleidx)}\big)  &\quad &\mbox{The $\sampleidx$-th data point within an indexed set of data points.} \nonumber \\ 
&y^{(\sampleidx)}  &\quad &\mbox{The label of the $i$th data point.} \nonumber \\ 
...
\item {\bf a sample}: a sequence of data points 
...
\item {\bf data point}: an elementary unit of information such 
...
\item {\bf labeled data point}. a data point for which we know the 
value of its label. 

\item {\bf dataset}: a collection (set or list) of data points. 
...
characterize a data point (e.g., the maximum amplitude of a 
...
feature space $\featurespace$ to the label space $\labelspace$. 
Given a data point with features $\vx$ we use a hypothesis map 
...
\item {\bf label}: some property of a data point which is of interest, 
such as the fact if a webcam snapshot shows a forest fire or not. In 
contrast to features, labels are properties of data points that cannot 
...
to guess or approximate the label of a data point based on its features. 
