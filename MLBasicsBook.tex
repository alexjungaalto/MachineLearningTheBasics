...
\caption{Three loss functions for assessing the quality of a hypothesis $h$ which 
	is used to classify a datapoint with label $y=1$ according to \eqref{equ_def_classification}. 
...
predicted label $\hat{y} \in \{-1,1\}$. A datapoint with features $\vx$, 
...
of the datapoint with feature vector $\vx^{(2)}$ seems to be much more reliable. 
...
datapoints are whose datasets; labels are correct partitioning/clustering; loss function is some notion of purity of 
...
\section{LinUCB}
\label{sec_lin_ucb}
datapoints are costumers characterized by feature vector; the label is 
...
whose nodes $\nodes$ represent individual datapoints which are connected by edges $\edges$ 
...
learning by solving the ERM sequentially as new datapoints come in. 
...
${\bm \mu}_{1}$, while for datapoints with label $y=-1$ the mean of $\vx$ is ${\bm \mu}_{-1}$. In 
...
minimum average loss (empirical error) on a set of labelled datapoints, 
...
\framebox[0.96\textwidth]{
 \parbox{0.9\textwidth}{
\begin{quote}
A golden rule of ML practice: try always to use different datapoints 
...
which applies broadly to different ML problems \cite{Larsen1999}.
 ...
 \begin{figure}[htbp]
   ...
   \includegraphics[width=0.7\textwidth]{TrainValSmall.jpg}  
    \caption{If we only have one single labeled dataset $\dataset$, we 
    	split it into a {\bf training set} $\dataset^{(\rm train)}$ and a {\bf validation set} 
    	$\dataset^{(\rm val)}$. We use the training set in order to 
    	learn (find) a good predictor $\hat{h}(\vx)$ by minimizing the 
    	empirical risk $\emperror(h| \dataset^{(\rm train)})$ (see \eqref{equ_def_ERM_funs}). 
    In order to validate the performance of the predictor $\hat{h}$ on 
    new data, we compute the empirical risk $\emperror(h| \dataset^{(\rm val)})$ 
    incurred by $\hat{h}(\vx)$ for the validation set $\dataset^{(\rm val)}$. 
    We refer to the empirical risk $\emperror(h| \dataset^{(\rm val)})$ obtained 
    for the validation set as the {\bf validation error}.}
...
conditioned on the dataset $\dataset$, the feature vector $\vx$ of a new datapoint (not belonging to 
...
applies if the observed datapoints are well modelled as realizations of random 
...
some labeled datapoints (training data) $\dataset= \{ \vz^{(\sampleidx)} = (\vx^{(\sampleidx)},y^{(\sampleidx)}) \}_{\sampleidx=1}^{\samplesize}$, 
...
So far, we only have discussed the statistical effect of 
...
for the notion of the degree $y^{(\sampleidx)}_{c}$ by which datapoint 
$\vx^{(\sampleidx)}$ belongs to cluster $c$. 

We define the degree $y^{(\sampleidx)}_{c}$ of datapoint $\vx^{(\sampleidx)}$ 
...
is obtained by considering each datapoint $\vx^{(\sampleidx)}$ being 
