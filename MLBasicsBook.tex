...
Let us illustrate the phenomenon of overfitting by considering linear regression that 
we discussed in Section \ref{sec_lin_regression}. Here, the goal is to learn a linear 
hypothesis $h(\vx) = \vw^{T} \mathbf{x}$ which is parametrized by the weight 
vector $\vw \in \mathbb{R}^{\featurelen}$. The hypothesis is used to predict the 
numeric label $y$ of a datapoint based on its feature vector $\vx \in \mathbb{R}^{\featurelen}$. 

Linear regression methods learn the weight vector $\vw$ of a linear predictor by 
minimizing the average squared error loss incurred on some training set 
 $$\dataset = \big\{ \big(\vx^{(1)},y^{(1)}\big),\ldots,\big(\vx^{(\samplesize)},y^{(\samplesize)}\big)  \big\}.$$ 
The training set consists of $\samplesize$ datapoints $\big(\vx^{(1)},y^{(1)}\big)$, 
...
Let us stack the feature vectors $\vx^{(\sampleidx)}$ and labels $y^{(\sampleidx)}$ of 
the training data into the feature matrix $\mX=(\vx^{(1)},\ldots,\vx^{(\samplesize)})^{T}$ 
and label vector $\vy=(y^{(1)},\ldots,y^{(\samplesize)})^{T}$.The optimal linear hypothesis  
is obtained for any weight vector $\widehat{\vw}$ that solves \eqref{equ_zero_gradient_lin_reg}. 
The corresponding minimum training error is obtained from \eqref{equ_emp_risk_lin_proje} as 
\begin{equation}
\emperror(h^{(\widehat{\vw})} \mid \dataset) = \min_{\vw \in \mathbb{R}^{\featuredim}} \emperror(h^{(\vw)}) = \|  (\mathbf{I}- \mathbf{P}) \vy \|^{2}.
\end{equation} 
...
$y - h^{(\vw_{\rm opt})}(\vx)$ on datapoints $(\vx,y)$ which are different from the 
