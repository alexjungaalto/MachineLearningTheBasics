...
\begin{algorithm}[htbp]
	...
	\begin{algorithmic}[1]
		\renewcommand{\algorithmicrequire}{\textbf{Input:}}
		\renewcommand{\algorithmicensure}{\textbf{Output:}}
		\Require   dataset $\dataset=\big\{ \big(\vx^{(1)}, y^{(1)}\big),\ldots,\big(\vx^{(\samplesize)}, y^{(\samplesize)}\big) \big\}$; split ratio $\splitratio$
		\State randomly shuffle the datapoint in $\dataset$
		\State create the training set $\trainset$ using the first $\samplesize_{t}\!=\! \lceil\splitratio \samplesize\rceil$ datapoints,
		$$\trainset = \big\{ \big(\vx^{(1)}, y^{(1)}\big),\ldots,\big(\vx^{(\samplesize_{t})}, y^{(\samplesize_{t})}\big) \big\}.$$
		\State create the validation set $\trainset$ by the $\samplesize_v = \samplesize - \samplesize_t$ remaining datapoints, 
		...
		\end{equation} 
		\State compute training error 
		\begin{equation} 
		\label{equ_def_training_error_val}
		E_{t} \defeq \emperror\big(\hat{h}| \trainset \big) = (1/\samplesize_{t}) \sum_{\sampleidx=1}^{\samplesize_{t}} \loss{(\vx^{(\sampleidx)},y^{(\sampleidx)})}{h}. 
		\end{equation} 
		\State compute validation error 
		\begin{equation} 
		\label{equ_def_training_val_val}
		E_{v} \defeq \emperror\big(\hat{h}| \valset \big)=   (1/\samplesize_{v}) \sum_{\sampleidx=\samplesize_{t}+1}^{\samplesize} \loss{(\vx^{(\sampleidx)},y^{(\sampleidx)})}{\hat{h}}. 
		\end{equation} 
...
of i.i.d. RVs whose common probability distribution is determined by $p(\vx,y)$, the choice of loss function and 
the hypothesis $\hat{h}$. If we know an upper bound $U$ on the variance of the (random) loss 
$\loss{(\vx^{(\sampleidx)},y^{(\sampleidx)})}{\hat{h}}$, we can bound the variance of $\valerror$ as 
$$ \sigma_{\valerror}^{2}  \leq U/\samplesize_{v}.$$ We can then ensure that the variance of the validation 
error does not exceed a given threshold $\eta$, say $\eta = (1/100) \trainerror^2$, 
...
and studied \cite{}. One very popular such variant, which is known as {\bf $k$-fold cross-validation} repeats the splitting into a training and a 
validation set $k$ times. During each repetition, this method uses different subsets for 
training and validation. We refer to \cite[Sec. 7.10]{hastie01statisticallearning} for a detailed 
discussion of $k$-fold cross-validation. 
