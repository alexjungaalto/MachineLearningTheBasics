...
still we evaluate the performance of $h(\vx)$ only using one single datapoint with $y=-1$. 
...
Consider learning a linear hypothesis $h( \vx) = \hat{\vw}^{T} \vx$ 
from datapoints which are modelled as realizations of i.i.d.\
random variables. If we interpret datapoints as realizations of 
random variables, also the weights $\hat{\vw}$ is a realization  
of a random variable. The bootstrap is a method for estimating 
(parameters of) its probability distribution $p(\hat{\vw})$.  
...
distribution $p(\hat{\vw})$. While the analysis in Section \ref{sec_gen_linreg} 
...
model we can compute the minimum possible expected loss or risk \ref{equ_def_risk}. 
...
which are different from the training set. % the general or average prediction performance of 
