...
\begin{figure}[htbp]
	...
	\vspace*{-13mm}
	\caption{
		Machine learning implements the scientific principle of iteratively 
		validating and refining a hypothesis about a ``real-world'' phenomenon. 
		This principle involves data, a model (or hypothesis space) and a loss function.}
	\label{fig_AlexMLBP}
\end{figure}
\end{center}
...
This book portrays ML methods as the combination of three basic 
...
ML methods differ vastly in their actual implementations 
...
systems. Nevertheless, deep learning methods and linear regression use 
...
On a lower-level, this tutorial helps ML engineers to 
...
which is required to manage a ML team. 
...
\end{equation} 
Each data point $\vz^{(i)} = \big(x^{(i)},y^{(i)}$ represents some previous day 
...
Figure \ref{fig_three_maps_example} depicts three particular maps $h^{(\mathbf{w})}$ 
...
to allow for more than one hypothesis. ML allows to choose (learn) 
...
data and the predictions obtained from a hypothesis map (see Figure \ref{}). 
