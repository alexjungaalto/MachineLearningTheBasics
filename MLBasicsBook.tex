...
in an application-specific sense. The extend of similarity between 
connected nodes $i,j \in \nodes$ is encoded in the edge weights $W_{i,j} > 0$ 
which are collected into the weight matrix $\mathbf{W} \in \mathbb{R}_{+}^{|\nodes| \times |\nodes|}$.  
