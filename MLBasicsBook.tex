...
every single microscopic property of a data point. 
Consider a data point that represents a vaccine. A full characterization of such a 
...
For an infinite hypothesis spaces, we cannot simply use the 
number of its elements as a measure for its size. Different 
concepts have been studied for measuring the size of infinite 
...
allow to use very simple but efficient iterative methods for solving 
the optimization problem underlying an ML method (see Chapter \ref{ch_GD}). 
}}
\end{center}
...
\item {\bf classification problem}: an ML problem involving 
...
snapshot shows a forest fire or not. In contrast to features, labels are properties of data points 
