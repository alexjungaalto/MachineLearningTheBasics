...
\begin{equation}
\label{equ_def_0_1}
\loss{(\vx,y)}{h} \defeq \begin{cases} 1 &\mbox{ if } y \neq \hat{y}  \\ 0 & \mbox{ else,} \end{cases} \mbox{ with } 
\hat{y} = 1 \mbox{ for }h(\vx) \geq 0\mbox{, and }
\hat{y} = -1 \mbox{ for }h(\vx) < 0.
\end{equation} 
...
are realizations of i.i.d.\ random variables.  The average $0/1$ loss on the left-hand side 
of \eqref{equ_0_1_approx_prob} is referred to as the {\bf accuracy} of the hypothesis $h$. 
...
In contrast, the shape of a map represented by a decision tree can be much more  
complicated. Using a sufficiently large (deep) decision tree, we can obtain a hypothesis 
map that closely approximates any given non-linear map. Using sufficiently deep decision trees 
for classification problems allows for highly irregular decision regions. 
...
\coordinate (A) at (-5,0);
\coordinate (optw) at (3,0) ; 
\coordinate (error) at (3,2); 
\coordinate (labelvec) at (3,4); 
\coordinate (B) at (5,0);
 \node[right] at (error)   (a) {$\| \vy - \mX \widehat{\vw}\|$};
\draw [dashed,line width=0.5pt]  ([xshift=-1cm]A)  --  ([xshift=-1cm]A) node [left]{$\{ \mathbf{X}\vw: \vw \in \mathbb{R}^{\featuredim}\}$} ;
\draw [dashed,line width=0.5pt] ([xshift=-1cm]A) -- ([xshift=1cm]B) ; 
\draw [->,,line width=2pt] (A) -- (labelvec) node[above right]{$\mathbf{y}$} ; 
\draw [dotted,,line width=2pt] (labelvec) -- (optw); 
\draw [->,,line width=2pt] (A) -- (optw) node[above right]{$\mathbf{X}\widehat{\vw}$} ; 
%\draw [extended line=0.5cm, stealth-stealth] (A) -- (B) node[pos=1.15,font=\small]{$\mathbf{X}\mathbf{w}$};     
%\draw [ -stealth] (0,0) -- (-2.6, 4.3) coordinate (yn) node[right]{label vector $\vy$}; 
...
%f(datapoin)= \min_{\vw \in \mathbb{R}^{\featuredim}} 
...
is huge. Another example of high-dimensional hypothesis spaces are deep learning methods 
whose hypothesis spaces are constituted by all maps represented by some ANN with {\bf billions of 
tunable weights}. 
...
%A ML method overfits if it learns a predictor $h \in \hypospace$ that 5
...
The validation error can also be used as a quality measure 
...
not require no detailed assumptions about the data distribution. Bootstrap methods 
allow to analyze statistical fluctuations in the learning process that may arise from 
using different training sets. The bootstrap allows for a more fine-grained diagnosis 
of ML methods than by relying only on a training and validation error. 

%Computing validation errors not only allows to detect overfitting and 
%select between different models (hypothesis spaces). 
As indicated in Figure \ref{fig_bars_val_sel}, for some ML applications, 
we might know a benchmark error level for ML methods. Such a benchmark level 
might be obtained from existing ML methods, human performance levels or 
from a probabilistic model (see Section \ref{sec_gen_linreg}). 
...
%verify if the ML method provides satisfactory prediction quality . 
...
%In general, linear regression overfits a training set of size $\samplesize$ as soon as the number $\featuredim$ of 
%features exceeds the sample size $\samplesize$,
\begin{equation} 
...
%The result \eqref{eq_zero_trianing_error} only applies if the feature vectors of 
...
%Section \ref{} A simple, yet quite useful, strategy to detect if a predictor $\hat{h}$ overfits the training 
...
\begin{figure}
	...
	%	\node [right] at (-9,8) {$ {\bm \Sigma}^{(3)}$} ; 
		\end{tikzpicture}
	\end{center}
	\caption{We split the dataset $\dataset$ into two subsets, a {\bf training set} $\trainset$ and a {\bf validation set} 
...
%$\hat{h}$ (e.g., via ERM \eqref{equ_def_ERM_funs}). 
%\vspace*{2mm}
%\begin{center}
%\framebox[0.96\textwidth]{
% \parbox{0.9\textwidth}{
...
%We refer to the first subset $\trainset$, of size $\samplesize_{t}$, as the {\bf training set}. 
%We refer to the second subset $\valset$, of size $\samplesize_{v}$, as the {\bf validation set}.  
%\item We learn a hypothesis $\hat{h}$ via ERM on the training set $\trainset$ (cf.\ \eqref{equ_def_ERM_funs}), 
%\begin{align} 
%\label{equ_def_hat_h_fitting}
...
and studied (see )\cite{Efron97} and Section \ref{sec_the_bootsrap}). 

One very popular modification of the single split into training and validation set 
is known as {\bf $k$-fold cross-validation} (CV) \cite[Sec. 7.10]{hastie01statisticallearning}. 
Figure \ref{fig_k_fold_CV} depicts the operations principle behind $k$-fold CV which is based on
dividing the entire dataset evenly into subsets or folds of same set. The training 
and validation of a hypothesis is then repeated for $k$ times. During each 
repetition, we use a different subsets for training and validation. The average (over 
all repetitions) validation error is a more robust estimator for the expected loss or risk 
compared to the validation error obtained from a single split. When using a single split 
into training and validation set, we might choose a very atypical set of datapoint for the 
validation set. Algorithm \ref{alg:kfoldCV_ERM} is obtained from Algorithm \ref{alg:validated_ERM} by replacing 
the single split into training and validation set by $k$-fold CV. 

\begin{figure}
\begin{center}
 \begin{tikzpicture}
\matrix (M) [matrix of nodes,
nodes={minimum height = 7mm, minimum width = 2.5cm, outer sep=0, anchor=center, draw},
...
{
	fold $1$ & $\valset\!=\!\dataset_{1}$ & & & & \\
	fold $2$ & & $\valset\!=\!\dataset_{2}$ & & & \\
	fold $3$ & & & $\valset\!=\!\dataset_{3}$ & & \\
	fold $4$ & & & &  $\valset\!=\!\dataset_{4}$ & \\
	fold $5$ & & & & &  $\valset\!=\!\dataset_{5}$ \\
};
\draw (M-1-2.north west) ++(0,2mm) coordinate (LT) edge[|<->|, >= latex] node[above]{dataset $\dataset=\big\{ \big(\vx^{(1)},y^{(1)}\big),\ldots, \big(\vx^{(\samplesize)},y^{(\samplesize)}\big) \big\}$} (LT-|M-1-6.north east);
\end{tikzpicture}
\end{center}
\caption{Illustration of $k$-fold CV for $k=5$. We evenly partition the entire dataset $\dataset$ into $k=5$ subsets 
	(or folds) $\dataset_{1},\ldots,\dataset_{5}$. We then repeat the validated ERM Algorithm \ref{alg:validated_ERM} for $k=5$ times. 
    The $\foldidx$th repetition uses the $\foldidx$th fold $\dataset_{\foldidx}$ as validation set and the remaining $k\!-\!1(=4)$ folds as training set.}
\label{fig_k_fold_CV}
\end{figure}

\begin{algorithm}[htbp]
	\caption{$k$-fold CV ERM}\label{alg:kfoldCV_ERM}
	\begin{algorithmic}[1]
		\renewcommand{\algorithmicrequire}{\textbf{Input:}}
		\renewcommand{\algorithmicensure}{\textbf{Output:}}
		\Require   model $\hypospace$, loss function $\mathcal{L}$, dataset $\dataset=\big\{ \big(\vx^{(1)}, y^{(1)}\big),\ldots,\big(\vx^{(\samplesize)}, y^{(\samplesize)}\big) \big\}$; number $k$ of folds
		\State randomly shuffle the datapoints in $\dataset$ \label{alg_shuffle_step_kCV}
		\State divide the shuffled dataset $\dataset$ into $k$ folds $\dataset_{1},\ldots,\dataset_{k}$ of size $B=\lceil\samplesize/k\rceil$, 
		\begin{equation}
		\hspace*{-12mm}\dataset_{1}\!=\!\big\{ \big(\vx^{(1)}, y^{(1)}\big),\ldots, \big(\vx^{(B)}, y^{(B)}\big)\} ,\ldots,\dataset_{k}\!=\!\big\{ \big(\vx^{((k\!-\!1)B+1)}, y^{((k\!-\!1)B+1)}\big),\ldots, \big(\vx^{(\samplesize)}, y^{(\samplesize)}\big)\}
		\end{equation} 
		\For{ fold index $\foldidx=1,\ldots,k$ }
	%	\State \texttt{<do stuff>}
		\State use $\foldidx$th fold as the validation set $\valset=\dataset_{\foldidx}$
		\State use rest as the {\bf training set} $\trainset=\dataset \setminus \dataset_{\foldidx}$
		\State  learn hypothesis $\hat{h}$ via ERM on the training set, 
		\begin{equation} 
		\label{equ_def_hat_h_fitting_cv}
		\hat{h}^{(\foldidx)} \defeq \argmin_{h\in \hypospace} \emperror\big(h| \trainset \big)
		\end{equation} 
		\State compute {\bf training error}
		\begin{equation} 
		\label{equ_def_training_error_val_cv}
		\trainerror^{(\foldidx)} \defeq \emperror\big(\hat{h}| \trainset \big) = (1/\big|\trainset\big|) \sum_{\sampleidx \in \trainset} \loss{(\vx^{(\sampleidx)},y^{(\sampleidx)})}{h}. 
		\end{equation} 
		\State compute {\bf validation error}
		\begin{equation} 
		\label{equ_def_training_val_val_cv}
		\valerror^{(\foldidx)} \defeq \emperror\big(\hat{h}| \valset \big)=   (1/\big|\valset\big|) \sum_{\sampleidx \in \valset} \loss{(\vx^{(\sampleidx)},y^{(\sampleidx)})}{\hat{h}}. 
		\end{equation} 
		\EndFor
		
		\State compute average training and validation errors 
		$$E_{t} \defeq (1/k) \sum_{\foldidx=1}^{k} 	\trainerror^{(\foldidx)}\mbox{, and }E_{v} \defeq (1/k) \sum_{\foldidx=1}^{k} 	\valerror^{(\foldidx)}$$
		\State pick a learnt hypothesis $\hat{h} \defeq 	\hat{h}^{(\foldidx)}$ for some $\foldidx \in \{1,\ldots,k\}$
		\Ensure learnt hypothesis $\hat{h}$; average training error $\trainerror$; average validation error $\valerror$
	\end{algorithmic}
\end{algorithm}
...
datapoint with $y=-1$. The validation error will then be dominated by the loss  
of $h(\vx)$ incurred only on datapoints with the label $y=1$. 

When learning and validating a hypothesis using imbalanced data, it might be useful to 
to generate synthetic datapoints to enlarge the minority class. This can be done using 
data augmentation techniques which we discuss in Section \ref{sec_data_augmentation}. 
Another option is to use a loss function that takes the different frequency of label values 
into account. 

Consider an imbalanced dataset of size $\samplesize=100$, which contains $90$ datapoints with 
label $y=1$ but only $10$ datapoints with label $y=-1$. We might then put more weight on wrong 
predictions obtained for the minority class (of datapoints with $y=-1$). This can be done by using 
a much larger value for the loss $\loss{(\vx,y=-1)}{h(\vx)=1}$ than for the loss $\loss{(\vx,y=1)}{h(\vx)=-1}$. 
Remember that the loss function is a design choice and can been freely set by the ML engineer. 


\section{Model Selection}
\label{sec_modsel}

Chapter \ref{ch_some_examples} illustrated how many well-known ML methods are obtained by different 
combinations of a hypothesis space or model, loss function and data representation. 
While the there might often be a natural choice for the loss function and data representation 
in a ML application, the right choice for the model is typically less obvious. This chapter shows 
how to use the validation methods of Section \ref{sec_validate_predictor} to choose 
between different candidate models. 

Consider datapoints characterized by a single numeric feature $x\in \mathbb{R}$ and numeric label $y\in \mathbb{R}$. If 
If we suspect that the relation between feature $x$ and label $y$ is non-linear, we might use 
polynomial regression which is discussed in Section \ref{sec_polynomial_regression}. Polynomial regression uses 
the hypothesis space $\hypospace_{\rm poly}^{(\featuredim)}$ with some maximum degree $\featuredim$. 
Different choices for the maximum degree $\featuredim$ yield a different hypothesis space: $\hypospace^{(1)} = \mathcal{H}_{\rm poly}^{(0)},\hypospace^{(2)} = \mathcal{H}_{\rm poly}^{(1)},\ldots,\hypospace^{(\nrmodels)} = \hypospace_{\rm poly}^{(M-1)}$. 

Another ML method that learns non-linear hypothesis map is Gaussian basis regression (see Section \ref{sec_linbasreg}). 
Here, different choices for the variance $\sigma$ and shifts $\mu$ of the Gaussian basis function \eqref{equ_basis_Gaussian} 
result in different hypothesis spaces. For example, $\hypospace^{(1)} = \mathcal{H}^{(2)}_{\rm Gauss}$ with $\sigma=1$ and $\mu_{1}=1$ and $\mu_{2}=2$, $\hypospace^{(2)} = \hypospace^{(2)}_{\rm Gauss}$ with $\sigma = 1/10$, $\mu_{1}=10$, $\mu_{2}= 20$.

Algorithm \ref{alg:model_selection} summarizes a simple method to choose between different candidate models $\hypospace^{(1)},\hypospace^{(2)},\ldots,\hypospace^{(M)}$. The idea is to first learn and validate a hypothesis, 
separately for each model, using Algorithm \ref{alg:kfoldCV_ERM}. For each model $\hypospace^{(\modelidx)}$, we 
lean a hypothesis $\hat{h}^{(\modelidx)}$ (via \eqref{equ_def_hat_h_fitting}) and compute its validation error 
$\valerror^{(\modelidx)}$ \eqref{equ_def_training_val_val}. We then choose the hypothesis $\hat{h}^{(\hat{\modelidx})}$ 
from those model $\hypospace^{(\hat{\modelidx})}$ which resulted in the smallest validation error $\valerror^{(\hat{\modelidx})} = \min_{\modelidx=1,\ldots,\nrmodels} \valerror^{(\modelidx)}$. 

The above model selection procedure has a very similar structure as ERM. The idea of 
ERM is to find the hypothesis with a set of different candidates (these candidates form a 
hypothesis space) based on a quality measure. For a particular hypothesis $h$ we use the (average) 
loss as a quality measure. Model selection uses the same principle but on a higher level. 
Instead of choosing or learning a hypothesis within a hypothesis space, we chose or learn 
a hypothesis space within a set of candidate hypothesis spaces. For a particular hypothesis 
space $\hypospace$ we the validation error \eqref{equ_def_training_val_val} of the hypothesis 
$\hat{h} \in \hypospace$ with minimum training error \eqref{equ_def_hat_h_fitting}. 

We have introduced validation techniques as a method to assess the quality of a  
hypothesis $\hat{h}$ that is learnt using ERM on a training set. The idea was to try out the 
hypothesis on a validation set which consists of datapoints that have not been 
used for learning or tuning the hypothesis $\hat{h}$. 

The model selection Algorithm \ref{alg:model_selection} learns a hypothesis $\hat{h}$ not 
only by using a training set for ERM (see \eqref{equ_def_hat_h_fitting_cv}). We also use the 
validation set in \eqref{equ_def_training_val_val_cv} to find the best model $\hypospace^{(\hat{\modelidx})}$ (see step \ref{step_pick_optimal_model})
which contains $\hat{h}$. Since we use the validation set to learn or 
tune the final hypothesis $\hat{h}$ delivered by Algorithm  \ref{alg:model_selection}, 
we cannot use the validation error \eqref{equ_def_training_val_val_cv} as a good indicator for the general 
performance of $\hat{h}$. 

We must try out the final hypothesis $\hat{h}$ on a test set which contains datapoints that have neither 
been used for ERM nor for validation of the different models $\hypospace^{(\modelidx)}$. This is implemented 
in the steps \ref{step_construct_test_set_mo_sel} and \ref{step_compute_test_error_mod_selection} of Algorithm \ref{alg:model_selection}. 


\begin{algorithm}[htbp]
	\caption{Model Selection}\label{alg:model_selection}
	\begin{algorithmic}[1]
		\renewcommand{\algorithmicrequire}{\textbf{Input:}}
		\renewcommand{\algorithmicensure}{\textbf{Output:}}
		\Require   list of candidate models $\hypospace^{(1)},\ldots,\hypospace^{(\nrmodels)}$, 
		loss function $\mathcal{L}$, dataset $\dataset=\big\{ \big(\vx^{(1)}, y^{(1)}\big),\ldots,\big(\vx^{(\samplesize)}, y^{(\samplesize)}\big) \big\}$; 
		number $k$ of folds, test fraction $\rho$
		\State randomly shuffle the datapoints in $\dataset$ \label{alg_shuffle_step_model-sel}
		\State determine size $\samplesize' \defeq \lceil \rho \samplesize \rceil$ of test set
		\State construct {\bf test set} \label{step_construct_test_set_mo_sel}
		$$\testset= \big\{\big(\vx^{(1)}, y^{(1)}\big),\ldots,\big(\vx^{(\samplesize')}, y^{(\samplesize')}\big) \big\}$$
		\State construct the set used for training and validation, 
		$$\dataset^{(\rm trainval)} = \big\{\big(\vx^{(\samplesize'+1)}, y^{(\samplesize'+1)}\big),\ldots,\big(\vx^{(\samplesize)}, y^{(\samplesize)}\big) \big\}$$
		\For{ model index $\modelidx=1,\ldots,\nrmodels$ }
		%	\State \texttt{<do stuff>}
		\State run Algorithm \ref{alg:kfoldCV_ERM} using $\hypospace=\hypospace^{(\modelidx)}$, dataset $\dataset=\dataset^{(\rm trainval)}$, loss $\mathcal{L}$ 
		and $k$ folds
		\State Algorithm \ref{alg:kfoldCV_ERM} delivers hypothesis $\hat{h}$ and validation error $\valerror$
		\State store learnt hypothesis $\hat{h}^{(\modelidx)}\defeq\hat{h}$ and validation error $\valerror^{(\modelidx)}\defeq\valerror$
	
		\EndFor
		
		\State pick model $\hypospace^{(\hat{\modelidx})}$ with minimum validation error 
		$\valerror^{(\hat{\modelidx})}\!=\!\min_{\modelidx=1,\ldots,M}\valerror^{(\modelidx)}$ \label{step_pick_optimal_model}
		
		\State define optimal hypothesis $\hat{h}= \hat{h}^{(\hat{\modelidx})}$ 
		
		\State compute {\bf test error}  \label{step_compute_test_error_mod_selection}
		\begin{equation} 
		\label{equ_def_training_error_val}
		\testerror \defeq \emperror\big(\hat{h}| \testset \big) = (1/\big|\testset\big|) \sum_{\sampleidx \in \testset} \loss{(\vx^{(\sampleidx)},y^{(\sampleidx)})}{\hat{h}}. 
		\end{equation} 
		
		
		\Ensure  hypothesis $\hat{h}$; training error $\trainerror^{(\hat{\modelidx})}$; validation error $\valerror^{(\hat{\modelidx})}$, 
		test error $\testerror$. 
	\end{algorithmic}
\end{algorithm}
%\begin{itemize} 
...
Sometimes it might be useful to use different loss functions for the training and 
the validation of a hypothesis. As an example, consider the methods logistic regression 
and SVM. Both methods learn a linear hypothesis but using two different loss functions, the 
logistic loss \eqref{equ_log_loss} and the hinge loss \eqref{equ_hinge_loss}, respectively. We then compute the 
validation errors using those two loss functions. 

It is difficult to compare these two methods since their validation errors are measured in different 
loss functions, the logistic loss \eqref{equ_log_loss} and the hinge loss \eqref{equ_hinge_loss}, respectively. We could 
instead compute the validation error for both methods, logistic regression and SVM, using the average $0/1$ 
loss \eqref{equ_def_0_1} (``accuracy''). The resulting average loss (accuracy) on the validation set can then be compared directly. 
...
%?????
%In the From Features to Classification slides, on page 15 (or 49) regarding 
%0.632 bootstrap, I don't understand where the formula 1 - (1 - 1/n)n-1 comes from.
%In the same lecture slides, page 17 (or 54) and the following page, I don't 
%understand what the numbers between training and testing mean, 
%as above it says that each instance should only be left out once.???????
...
\begin{center}
    \begin{tikzpicture}
             ...
             \draw[red, ultra thick, domain=0:5] plot (\x,  {2*\x/(1+\x)}) node [above] {bias of $\widehat{\vw}^{(\lambda)}$};
              \draw[blue, ultra thick, domain=0:5] plot (\x,  {2/(1+\x)}) node [above]  {variance of $\widehat{\vw}^{(\lambda)}$};
...
\item Issue 2: We need to handle the situation when after a cluster assignment update  \eqref{equ_cluster_assign_update}, 
there is a cluster $\clusteridx$ with no datapoints associated with it, $|\{ \sampleidx: y^{(\sampleidx)} = \clusteridx \}|=0$. 
For such a cluster $\clusteridx$, the update \eqref{equ_cluster_mean_update} is not well-defined. 
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=9cm]{EvolKmeans.eps}  
\vspace*{-6mm}
\end{center}
\caption{Evolution of cluster means (large dot and large cross) and cluster assignments during the iterations of 
	$k$-means.}
...
to be the actual ``correct'' covariance matrix for cluster $\clusteridx$. %The estimates $\mathbf{C}^{(c)}$ 
